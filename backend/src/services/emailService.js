...
function getTransporter() {
  ...
  if (!transporter) {
    transporter = nodemailer.createTransport({
      ...
      // Without these, a network issue reaching the SMTP host leaves
      // sendMail() hanging (nodemailer's own default is ~2 minutes) instead
      // of failing fast — which otherwise stalls the whole request (e.g.
      // forgot-password) well past the frontend's own timeout.
